...
#' weighted mean. `NA` components are handled in one of two ways depending on
#' what their absence actually means:
#' \itemize{
#'   \item `standard_adduct_recovery_score` and `eips_evidence_score` are only
#'     ever evaluable under a structural precondition (standards-library
#'     membership, and the `iso_morphology_candidates()`/C13 gating in
#'     [eips_score()], respectively) - `NA` there means the check did not
#'     apply, not that it was made and failed. These two are excluded from
#'     the calculation and the remaining weights renormalized to sum to 1,
#'     exactly as before.
#'   \item `mass_error_score`, `adduct_spatial_score`,
#'     `isotope_evidence_score` and `family_coherence_score` are evaluable
#'     for essentially any candidate (a second adduct form, a corroborating
#'     family member, or a C13 isotope pattern could in principle exist for
#'     any organic compound with carbon). `NA` there is real negative
#'     evidence - it was looked for and not found - so it is treated as a
#'     literal `0` and its weight stays in the average. `mass_error_score` is
#'     included in this group only for conceptual consistency; in practice it
#'     is essentially never `NA`, since a candidate only reaches this
#'     function after already matching on mass.
#' }
#' This means a `broad_db_only` candidate supported only by mass (no isotope,
#' adduct, family or EIPS evidence) no longer reaches a `priority_score` near
#' its raw `mass_error_score` - the three zero-filled evidence components
#' actively pull it down instead of dropping out of the average.
#'
...
compute_priority_score <- function(scores, weights = default_priority_weights()) {
  ...
  # Only these two are ever structurally inapplicable (see @description);
  # `NA` for the rest is real negative evidence and is zero-filled below
  # instead of excluded, so their weight is never dropped from the average.
  renormalize_on_na <- c("standard_adduct_recovery_score", "eips_evidence_score")
  zero_on_na <- intersect(score_cols, setdiff(score_cols, renormalize_on_na))

  zero_sub <- mat[, zero_on_na, drop = FALSE]
  zero_sub[is.na(zero_sub)] <- 0
  mat[, zero_on_na] <- zero_sub
